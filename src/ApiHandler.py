...
class ApiHandler(): 
     ...
     + ' return tokens!'+ \
     ' General instruction: When there are no pre- or postconditions, instead of writing "true", find one variable in the code ' + \
     ' and use it to write a true expression for example "x=x".'

     systemMsg = "You are a helpful assistant, can work with code and give precise answers."
    
     base_prompt = 'In the following, I will provide a code snippet encased in ```c and ``` which may or may ' + \
     ...
     code2NL_prompt = 'In the following, a code snippet encased in ```c and ``` will be provided which may or may not be buggy. ' +\
          ...
          'It should contain all necessary information, but no code pieces.'
     
     NL2spec_prompt = 'In the following, I will provide a natural language description of a code snippet. ' +\
     ...
     def gen_nl_spec_request(cls, lines, prompt_choice, llm):
          nl_response = cls.get_nl_explanation(lines, llm)
